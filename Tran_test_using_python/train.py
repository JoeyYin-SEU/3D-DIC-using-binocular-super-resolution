...
def train(train_loader, cfg):
    ...
    if cfg.using_epipolar:
        cfg.model_path='log/using_ep/iPASSR.pth.tar'
    else:
        cfg.model_path='log/non_ep/iPASSR.pth.tar'
        ...
        if idx_epoch % 1 == 0:
            ...
            if cfg.using_epipolar:
                torch.save({'epoch': idx_epoch + 1, 'state_dict': net.state_dict()},
                           'log/using_ep/' + cfg.model_name + '_' + str(cfg.scale_factor) + 'xSR_epoch' + str(idx_epoch + 1) + '.pth.tar')
            else:
                torch.save({'epoch': idx_epoch + 1, 'state_dict': net.state_dict()},
                           'log/non_ep/' + cfg.model_name + '_' + str(cfg.scale_factor) + 'xSR_epoch' + str(idx_epoch + 1) + '.pth.tar')
            loss_epoch = []
        if idx_epoch==(cfg.n_epochs-1):
            if cfg.using_epipolar:
                torch.save({'epoch': idx_epoch + 1, 'state_dict': net.state_dict()},
                           'log/using_ep/' + cfg.model_name + '_' + str(cfg.scale_factor) + 'xSR.pth.tar')
            else:
                torch.save({'epoch': idx_epoch + 1, 'state_dict': net.state_dict()},
                           'log/non_ep/' + cfg.model_name + '_' + str(cfg.scale_factor) + 'xSR.pth.tar')



def main(cfg):
    if cfg.using_epipolar:
        cfg.trainset_dir='./data/train/using_ep'
    else:
        cfg.trainset_dir='./data/train/non_ep'
